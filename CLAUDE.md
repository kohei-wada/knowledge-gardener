...
- **Hooks**:
  ...
  - `Stop` (Phase 3 of issue #1, **opt-in via `KG_AUTO_RECAP=1`**, registered with `async: true`) runs `recap/autorecap/` to silently maintain one coalesced block per session (keyed by bare `kg-recap-sid:{sid8}`) in today's daily note. The block has two layers: an append-only mechanical `### Timeline` (written on every Stop) and a `### KPT` section regenerated from the conversation transcript only on "substantive" Stops. A Stop is substantive if it produced a durable change (Edit/Write, git commit/push, Agent dispatch), OR tool-call count >= `KG_RECAP_MIN_CALLS` (default 5), OR duration >= `KG_RECAP_MIN_MINUTES` (default 5). Non-substantive Stops append Timeline only and spend no LLM call. Fire-and-forget — Claude returns control to the user immediately while the recap composes in the background. Default-off so OSS installs don't surprise users with auto-commits.
- **Vault location**: Resolved from `KG_VAULT` env var at runtime; not stored in this repo
- **Session log location**: `$XDG_STATE_HOME/knowledge-gardener/sessions/` (fallback `~/.local/state/`). Machine-local derived state, not vault content. `garden-recap` consumes these logs via `recap/aggregate/` since `v0.9.0` (Phase 2); falls back to recollection when no logs exist. Since `v0.10.0` (Phase 3) the `Stop` hook can silently auto-write the same recap when `KG_AUTO_RECAP=1` (orchestrator at `recap/autorecap/`). Since `v0.17.0`, the manual `garden-recap` skill reuses the same two-layer `kg-recap-sid` block via `recap.manual_recap` CLI (no headless claude — the assistant authors the KPT), so manual wrap-ups coalesce with the auto block rather than creating a separate entry.
