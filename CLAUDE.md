...
- **Session log location**: `$XDG_STATE_HOME/knowledge-gardener/sessions/` (fallback `~/.local/state/`). Machine-local derived state, not vault content. `garden-recap` consumes these logs via `recap/aggregate/` since `v0.9.0` (Phase 2); falls back to recollection when no logs exist. Since `v0.10.0` (Phase 3) the `Stop` hook can silently auto-write the same recap when `KG_AUTO_RECAP=1` (orchestrator at `recap/autorecap/`).
