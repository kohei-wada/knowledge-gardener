...
- **Hooks**:
  ...
  - `Stop` (Phase 3 of issue #1, **opt-in via `KG_AUTO_RECAP=1`**, registered with `async: true`) runs `recap/autorecap/` to silently spawn headless Claude, generate today's session block, and `git commit && git push` to the vault. Fire-and-forget — Claude returns control to the user immediately while the recap composes in the background. Default-off so OSS installs don't surprise users with auto-commits.
